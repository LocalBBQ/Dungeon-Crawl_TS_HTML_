...
// drawOne(context, { obstacle, obstacleManager }) draws a single obstacle (used for Y-sorted interleave with entities).
class ObstacleLayerRenderer {
    /** Draw a single obstacle. Used when interleaving depth obstacles with entities by Y. */
    drawOne(context, data) {
    ...
    _drawObstacleBody(ctx, canvas, camera, zoom, obstacle, obstacleManager, useEnvironmentSprites) {
        const screenX = camera.toScreenX(obstacle.x);
        const screenY = camera.toScreenY(obstacle.y);
        const w = obstacle.width * zoom;
        const h = obstacle.height * zoom;
