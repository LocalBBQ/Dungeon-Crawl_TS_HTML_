...
export interface ObjectConfig {
  minSize: number;
  maxSize: number;
  defaultSpritePath: string | null;
  color: string;
  /** If true, obstacle can be broken by player/enemy attacks. */
  breakable?: boolean;
...
export class ObjectFactory {
  configs: Record<string, ObjectConfig>;
  ...
  constructor() {
    this.configs = {
      ...
      bush: { minSize: 25, maxSize: 35, defaultSpritePath: 'assets/bush.png', color: '#3a5a2a', breakable: true, hp: 5 },
  ...
  getConfig(type: string): ObjectConfig {
  ...
  createObjectsInArea(
    ...
    type: string,
    count: number,
    excludeArea: { x: number; y: number; radius: number } | null = null
  ): CreatedObject[] {
  ...
  createObjectsAlongBorder(
    worldWidth: number,
    worldHeight: number,
    type: string,
    spacing: number,
    border: 'all' | 'top' | 'bottom' | 'left' | 'right' = 'all'
  ): CreatedObject[] {
    const objects: CreatedObject[] = [];
    const config = this.getConfig(type);
...
if (typeof window !== 'undefined') {
  (window as Window).ObjectFactory = ObjectFactory;
}
