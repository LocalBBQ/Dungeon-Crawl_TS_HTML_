...
export class ChestRenderer {
    render(context: RenderContext, data: {
        ...
        if (interactionPromptOnly) {
            if (!playerNearChest) return;
            this.drawChestPrompt(ctx, screenX, screenY, w, h);
            return;
        }
        ...
        if (!skipInteractionPrompt && playerNearChest) {
            this.drawChestPrompt(ctx, screenX, screenY, w, h);
        }
    }

    private drawChestPrompt(ctx: CanvasRenderingContext2D, screenX: number, screenY: number, w: number, h: number): void {
